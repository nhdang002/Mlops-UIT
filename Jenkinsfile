...
node('main'){
    ...
    stage('Security gate'){
        for(int i = 0;i < 5;i++) {
            if (TOOL_FAILED[i]){
                mail to: 'ngohaidang2020@gmail.com', subject: "Status of pipeline: ${currentBuild.fullDisplayName}", 
    ...
    node('kubectl') {
        ...
        stage('Deploy to production') {
            container('kubectl') {
                ...
                mail to: '20521157@gm.uit.edu.vn', subject: "Status of pipeline: ${currentBuild.fullDisplayName}", 
